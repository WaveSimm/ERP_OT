...
> **Version**: V2 (Design **v0.5** — 카테고리 도메인 폐기 + 사업(계약)/구분 도입 + 결재 분리)
> **Author**: 오션테크 + Claude
> **Date**: 2026-05-19 (v0.5)
> **Status**: 구현 반영 (commit fb8a184 + 0fbb389 + v1.6.4) + Design 동기화 완료
> **Planning Doc**: [경비정산-ERP통합.plan.md](../01-plan/features/경비정산-ERP통합.plan.md)
> **Analysis**: [경비정산-ERP통합.analysis.md](../03-analysis/경비정산-ERP통합.analysis.md) (v2 Match 96% → v0.5 갱신으로 98%+ 회복 예상, 재분석 필요)
...
```
                        ...
                        │   /expense/* (6 pages: dashboard/transactions/  │
                        │     receipts/settlements/sources/finance)       │
                        │   /me/dashboard (경비 카드 추가)                │
                        │   ※ v0.5: /expense/categories 폐기              │
                        └────────────────┬────────────────────────────────┘
...
4. 사용자: 매칭 confirm + 분류 (v0.5: 카테고리 → 사업/구분)
   PATCH /api/v1/expense/transactions/:id { contractId?, contractNumber?, contractName?, detail?, memo? }
   PATCH /api/v1/expense/matches/:id { confirmedAt }

5. 사용자: 빈 정산 묶음 생성 + 거래 할당 (v1.6.4: 자동 제목 폐기, 사용자 직접 입력)
   POST /api/v1/expense/settlements/empty { title }
   PATCH /api/v1/expense/settlements/transactions/:transactionId { settlementId }
   → settlement: DRAFT
   → exceljs로 회사 양식 Excel 생성 (s3 또는 local)

6. 사용자: 결재 상신 (v1.6.4 — 결재 분리, expense에서 직접 상신 라우트 폐기)
   /approval/new 또는 결재 페이지에서 EXPENSE 양식 + referenceType=EXPENSE_SETTLEMENT 작성
   → approval-service POST /api/v1/approval/documents
   → 결재 생성 직후 approval → expense /internal/settlements/:id/link-approval webhook 호출
   → settlement.status = SUBMITTED, approvalDocumentId 기록

7. 결재자: 결재 진행 (기존 ERP 결재 화면)
   approval-service: 단계별 승인 → 최종 APPROVED
   → HTTP webhook: approval → expense /internal/settlements/from-approval

8. expense-service from-approval webhook handler:
...
### 2.3 Dependencies (v0.5)

| 의존 | 방향 | 통신 | 목적 |
|---|---|---|---|
| expense → auth | HTTP | `/internal/users/*` | 사용자/부서 검증, 결재라인 조회 |
| expense → ocr | HTTP | `/api/v1/ocr/scan` | 영수증 OCR |
| **expense → equipment (logical)** | **DB 참조 (FK 없음)** | `contractId`, `contractNumber`, `contractName` snapshot | 거래의 사업(계약) 연결 — v0.5 신규. 무결성은 application layer 책임 |
| approval ← expense | HTTP | `POST /internal/documents` | (v1.6.4 이전) expense에서 자동 상신. **v1.6.4 이후 폐기** — 결재는 /approval/new에서 직접 작성 |
| **approval → expense (link)** | **HTTP webhook** | `POST /internal/settlements/:id/link-approval` `POST /internal/settlements/unlink-approval` | 결재 생성·삭제 시 settlement 연결/해제 (v1.6.4 신규) |
| **approval → expense (status sync)** | **HTTP webhook** | `POST /internal/settlements/from-approval` | 결재 완료/반려 동기화 (v0.4 변경: AMQP → HTTP) |
...
// v0.5 (2026-05-15): 카드/현금 소유 구분 — 정산 구분 (개인/법인)
// ※ v0.4의 CategoryScope enum은 폐기 (ExpenseCategory 모델 제거와 함께)
enum SourceOwnership {
  PERSONAL    // 개인 부담 — 환급 대상
  CORPORATE   // 법인카드 — 회사 결제, 환급 없음
...
model ExpenseSource {
  ...
  displayName String?         // 대표이름 (사용자 자유입력 별칭) — v0.5 신규
  type        SourceType
  cardNumber  String?
  ownership   SourceOwnership @default(PERSONAL)  // v0.5 신규
...
model ExpenseTransaction {
  ...
  // 분류 (v0.5: 카테고리 폐기 → 사업(계약) + 상세내역으로 교체)
  contractId        String?           // equipment.Contract.id (cross-service, FK 없음)
  contractNumber    String?           // snapshot: 계약번호 (예: 26-12)
  contractName      String?           // snapshot: 계약명
  ...
  @@index([userId, contractId])         // v0.5: categoryId 인덱스 → contractId 인덱스로 교체
  @@index([userId, status])
  @@map("transactions")
  @@schema("expense")
}
...
// ⚠️ v0.5 — ExpenseCategory 모델 폐기 (commit 0fbb389, 2026-05-18)
// 분류는 ExpenseTransaction.contractId/contractName + detail 필드로 대체.
// /admin/categories, /api/v1/expense/categories 라우트 및 /expense/categories 페이지도 모두 폐기.

// ⚠️ V2 확장 — 결재·재무팀 흐름 통합 (v0.5: 카테고리 의존 제거 + 자동 제목 폐기)
model ExpenseSettlement {
  id                 String           @id @default(cuid())
  userId             String

  periodStart        DateTime?        // v0.5: 자동 생성 폐기 → nullable
  periodEnd          DateTime?        // v0.5: nullable

  title              String           // v1.6.4: 사용자가 직접 입력 (자동 제목 폐기)
  ...
  categoryStats      Json?            // legacy 잔재. 통계용 (사업/구분별 합계로 재해석 가능)

  // v0.5: 카테고리 코드 (legacy — 자동 분리 시대 잔재. 수동 모드에선 nullable)
  categoryCode       String?
  ...
  // v0.5: @@unique([userId, periodStart, periodEnd]) 제거 (자동 기간 분리 폐기)
...
model ExpenseSettlementItem {
  ...
  // v0.5: categoryOverrideId 폐기 (ExpenseCategory 제거에 따라)
  memoOverride       String?
...
ExpenseSource ──→ ExpenseStatement ──→ ExpenseTransaction
   (ownership)                          │
                                        │  ※ v0.5: ExpenseCategory FK 제거
                                        ├╌→ equipment.Contract (logical link via contractId, FK 없음)
...
# Sources (카드/현금 마스터)
GET    /sources
POST   /sources             { type, name, displayName?, cardNumber?, ownership? }
PATCH  /sources/:id         { displayName?, ownership?, active? }
DELETE /sources/:id
...
# Transactions (거래) — v0.5: categoryId 폐기 → contractId/detail
GET    /transactions?status=&contractId=&from=&to=
POST   /transactions        (수동 입력)
GET    /transactions/:id
PATCH  /transactions/:id    { contractId?, contractNumber?, contractName?, detail?, memo?, status? }
...
# Categories — v0.5 전체 폐기 (ExpenseCategory 모델 제거, commit 0fbb389)
# 분류는 contractId/detail 필드로 대체

# Settlements (정산 + 결재) — v1.6.4 결재 분리 반영
GET    /settlements?status=&page=&limit=
GET    /settlements/:id
POST   /settlements/empty                       { title }              # 빈 정산 묶음 생성 (수동 워크플로우)
PATCH  /settlements/transactions/:transactionId { settlementId|null }  # 거래의 정산 할당/해제
PATCH  /settlements/:id/title                   { title }              # 제목 수정
DELETE /settlements/:id                                                # DRAFT만
GET    /settlements/:id/excel                                          # 회사 양식 Excel 다운로드
GET    /settlements/me/summary                                         # 내대시보드 카드 요약

# ※ v1.6.4 (2026-05-16) 폐기된 라우트:
#   POST /settlements                       — periodStart/periodEnd로 자동 묶음 생성 (legacy, createForPeriodByCategory)
#   POST /settlements/:id/submit            — expense에서 직접 결재 상신
#   결재 작성·취소는 이제 /approval/new 또는 결재 상세 페이지에서 수행하고,
#   approval-service가 /internal/settlements/:id/link-approval webhook으로 연결합니다.
```
...
```
# (v0.5 폐기) 표준 카테고리 관리 — ExpenseCategory 모델 제거로 라우트 자체 폐기
# GET/POST/PATCH/DELETE /admin/categories     ← 폐기

# (백로그) 전사 정산 감사 조회 — Gap G-4
# GET /admin/settlements?userId=&status=      ← 미구현, 사용자 명시 요청 시 진행
```

권한 (구현 시): ADMIN role 만.

### 5.4 Internal (`/internal/*`, x-internal-token)

v0.5 현재 백로그 상태 (다른 서비스에서 재사용 요청이 없어 YAGNI 보류):

```
# (백로그) 다른 서비스용 정산 요약 — Gap G-6
# GET /internal/expense/settlement-summary?userId=    ← 미구현, 실 사용처 발생 시 추가
```

내대시보드 카드는 사용자 토큰으로 `GET /api/v1/expense/settlements/me/summary` 사용 (§5.1).

### 5.5 Internal Webhook Endpoints (expense-service 측) — v0.5

v0.3 명세는 RabbitMQ Subscriber 였으나, **2026-05-12 결재 후속 통합 PDCA 결정으로 HTTP webhook 방식으로 변경**되었습니다. RabbitMQ는 활동 로그 전용으로만 사용. **v1.6.4 (2026-05-16)** 결재 분리 결정으로 link-approval / unlink-approval webhook이 추가되었습니다.

```
POST   /internal/settlements/:id/link-approval  (결재 생성 시 settlement 연결 — v1.6.4 신규)
POST   /internal/settlements/unlink-approval    (결재 삭제 시 settlement 해제 — v1.6.4 신규)
...
#### 5.5.0 `POST /internal/settlements/:id/link-approval` · `POST /internal/settlements/unlink-approval` (v1.6.4)

결재 작성·취소가 `/approval/new`에서 수행되므로(v1.6.4 결재 분리), approval-service가 결재 문서를 생성·삭제할 때 expense-service에 연결을 알립니다. `referenceType = "EXPENSE_SETTLEMENT"` 결재 문서만 대상.

```typescript
// link-approval (path :id 는 settlementId)
// Request body
{ approvalDocumentId: string }
// 처리: settlement.approvalDocumentId 기록 + status = SUBMITTED + submittedAt 기록

// unlink-approval
// Request body
{ approvalDocumentId: string }
// 처리: approvalDocumentId 키로 settlement 조회 → approvalDocumentId 해제 + status 복귀 (DRAFT)
```
...
```
approval-service ─[HTTP POST]─→ expense-service /internal/settlements/:id/link-approval  (v1.6.4 결재 생성)
approval-service ─[HTTP POST]─→ expense-service /internal/settlements/unlink-approval    (v1.6.4 결재 삭제)
approval-service ─[HTTP POST]─→ expense-service /internal/settlements/from-approval      (결재 완료/반려)
재무 송금 모듈   ─[HTTP POST]─→ expense-service /internal/settlements/from-payment       (송금 처리)
재무 송금 해제   ─[HTTP DELETE]→ expense-service /internal/settlements/from-payment       (송금 해제)
expense-service ─[AMQP publish]→ activity-log queue                                       (활동 로그 전용)

※ v1.6.4 폐기: expense-service → approval-service /internal/documents (결재 자동 상신)
   결재 문서 작성은 /approval/new 에서 직접 수행됩니다.
```
...
### 6.3 동일 양식 내 분기 처리 (v0.5 — SourceOwnership 활용)
...
v0.5에서는 거래 입력 시점에 **`ExpenseSource.ownership`** 으로 환급 대상을 분리합니다:

| ownership | 처리 |
|---|---|
| `PERSONAL` | 개인카드/현금 — 정산묶음 작성·결재·송금까지 전체 워크플로우 진입 |
| `CORPORATE` | 법인카드 — 회사가 직접 결제. 거래 import는 허용하되 정산묶음 환급 대상에서 자동 제외. 영수증 보관·증빙용으로만 사용 |

수기 작성 차단 로직 불필요 (양식이 하나뿐). frontend dropdown 필터링도 불필요 (v0.4 잔여 EXPENSE_CLAIM 분기 코드는 dead code — v0.5에서 정리 완료).
...
## 7. UI Wireframes (v0.5)

### 7.1 페이지 구성 (`apps/web/src/app/expense/`)

```
/expense                     ─ 대시보드 (요약 카드: 정산대상/완료/제외/전체)
/expense/transactions        ─ 거래 리스트 + 명세서 import
                              + 사업(계약)/구분(ownership)/상세내역 필드 입력
/expense/receipts            ─ 영수증 리스트 + 업로드
/expense/matches             ─ 매칭 confirm UI
/expense/settlements         ─ 정산 목록
/expense/settlements/[id]    ─ 정산 상세 + 결재 진행 추적
                              (v1.6.4: 결재 상신은 /approval/new 로 이동)
/expense/sources             ─ 카드/현금 마스터 관리 (ownership 포함)
/expense/finance             ─ 재무팀 큐 (역할 제한)

# ※ v0.5 폐기: /expense/categories — ExpenseCategory 모델 제거에 따라
```

### 7.0 매뉴얼 (사용자 안내) — `apps/web/public/manual-expense/index.html` v1.4

매뉴얼은 단일 소스(public 폴더)로 관리됩니다 (`project_manual_single_source` 정책).

| 매뉴얼 변경 (v1.4, 2026-05-18) | 반영 사유 |
|---|---|
| 카테고리 도메인 폐기 → 사업(계약)·구분·상세내역 도입 | commit 0fbb389 |
| 정산분류 4옵션 (개인/법인/제외/전체) | SourceOwnership + status 조합 |
| 자동 제목 폐기 → 사용자 직접 제목 입력 | v1.6.4 settlement 결재 분리 |
| 탭 단순화 (요약 카드 5개 → 4개) | UX 개선 |
| §11 지출결의서 작성 가이드 인라인 통합 | 결재 양식별 매뉴얼 분리 백로그 (`project_manual_approval_backlog`) |
| 법인카드 → 지출결의서 양식 안내 | 환급 vs 회사 결제 구분 명확화 |

### 7.2 핵심 와이어프레임 (텍스트)

#### `/expense/settlements/[id]` — 정산 상세 (v0.5)

```
┌──────────────────────────────────────────────────────────────────┐
│  ← 정산 목록     2026년 4월 출장 정산 (사용자 직접 입력 제목)      │
│  ╭────────────────────────────────────────────────────────────╮  │
...
│  ───────────────────────────────────────────────────────────────  │
│  ✅ 작성 완료 (04/30 18:00)                                      │
│  ✅ 결재 상신 (05/01 09:00 · /approval/new 에서 작성)             │
│  ◐  결재 진행 중                                                  │
│      └─ ✅ 부서장 승인 (05/01 14:30)                              │
│      └─ ◐  회계팀 검토 중                                         │
│  ⬜ 재무팀 접수                                                   │
│  ⬜ 입금 완료                                                     │
│                                                                  │
│  [Excel 다운로드]  [제목 수정]  [결재 상신 →]                     │
│   (결재 상신 버튼은 /approval/new 로 이동, EXPENSE 양식 사전 선택) │
│                                                                  │
│  거래 목록 (47)                                                   │
│  ───────────────────────────────────────────────────────────────  │
│  사업(계약)   │ 구분   │ 일시        │ 가맹점    │ 금액   │ 상세    │
│  26-12 OO    │ 개인   │ 04/01 12:30 │ 김밥천국  │ 8,500 │ 점심회의 │
│  ...                                                              │
└──────────────────────────────────────────────────────────────────┘
```
...
`GET /api/v1/expense/settlements/me/summary` 응답을 기반으로 표시합니다.
...
### 12.1bis v0.5 후속 작업 — 완료됨

> **상태**: 2026-05-16 ~ 05-18 완료 (commit `0fbb389` 외). expense는 pre-prod 단계라 마이그 부담 없이 drop & rebuild 진행.

1. **v1.6.4 결재 분리 (2026-05-16)**: expense settlement submit/cancel 라우트 제거 → `/approval/new` 에서 직접 작성. `/internal/settlements/:id/link-approval`, `/internal/settlements/unlink-approval` webhook 신규
2. **v1.6.1 SourceOwnership 도입 (2026-05-15)**: `ExpenseSource.ownership` 컬럼 추가 (PERSONAL/CORPORATE)
3. **v1.6.2 사업(계약) 연결 (2026-05-15)**: `ExpenseTransaction` 에 `contractId`/`contractNumber`/`contractName` snapshot + `detail` 컬럼 추가. `categoryId` 인덱스 → `contractId` 인덱스 교체
4. **카테고리 도메인 폐기 (2026-05-18, commit `0fbb389`)**: `ExpenseCategory` 모델·라우트·서비스·시드 + `/expense/categories` 페이지 + 관련 admin 라우트 모두 제거. `ExpenseSettlement.categoryStats`/`categoryCode` 만 legacy 잔재로 남김 (통계용)
5. **자동 제목 폐기 (v1.6.4)**: settlement title은 사용자 직접 입력. `ExpenseSettlement.periodStart/periodEnd` nullable 전환, `@@unique([userId, periodStart, periodEnd])` 제거
6. **매뉴얼 v1.4 배포 (2026-05-18)**: `apps/web/public/manual-expense/index.html` 갱신 — §1~§10 갱신 + §11 지출결의서 가이드 인라인 통합
...
| Risk | 완화 |
|---|---|
| 순환 의존 (approval ↔ expense) | v1.6.4 이후 approval → expense webhook(link/unlink/from-approval) 만 사용. expense → approval HTTP 호출 폐기. |
| ~~RabbitMQ 메시지 유실~~ | **v0.4에서 AMQP subscriber 폐기**. HTTP webhook은 4xx 재시도 + 응답 실패 시 approval 측 큐 보관 |
| webhook 호출 실패 (link/from-approval/from-payment) | 멱등 처리 + approval 측 정기 reconcile (DOC ↔ settlement.approvalDocumentId 매칭, 불일치 시 재전송) |
| cross-service contractId 무결성 | equipment.Contract 삭제 시 expense_transactions.contractId 고아 발생 가능. Application layer에서 조회 시 contractName/contractNumber snapshot으로 표시 (snapshot 컬럼이 안전망) |
...
| ~~표준 카테고리 코드 충돌~~ | **v0.5 ExpenseCategory 모델 폐기로 해소** |
...
### O3. 정산 묶음 분할 (v0.5 — 해결)
- v0.4까지: 같은 기간 내 1개 제한 (`@@unique([userId, periodStart, periodEnd])`)
- **v0.5 변경**: 자동 묶음 폐기 + 사용자 직접 제목 입력 → unique 제약 제거. `periodStart/periodEnd` nullable. 사용자가 임의 묶음 개수 생성 가능.
...
| **0.5** | **2026-05-19** | **카테고리 도메인 폐기 + 사업(계약)/구분 도입 + 결재 분리 반영 (analyze v2 Match 96% → 98%+ 회복 목적, code → design 동기화)**: ① §3 schema — **`ExpenseCategory` 모델·`CategoryScope` enum 전체 제거** (commit `0fbb389`, 2026-05-18). `ExpenseTransaction`의 `categoryId` → **`contractId`/`contractNumber`/`contractName` snapshot + `detail`** 교체 (equipment.Contract cross-service, FK 없음). `@@index([userId, categoryId])` → `@@index([userId, contractId])`. **`SourceOwnership` enum 신규** (PERSONAL/CORPORATE) + `ExpenseSource.ownership` 컬럼 추가. ② `ExpenseSettlement` — periodStart/periodEnd nullable + `@@unique([userId, periodStart, periodEnd])` 제거 (v1.6.4 자동 묶음 폐기). `categoryCode`/`categoryStats`는 legacy 통계용으로만 잔존. ③ §5 API — `/categories` 라우트 전체 폐기, `/admin/categories` 라우트 폐기. `POST /settlements`(legacy createForPeriodByCategory) + `POST /settlements/:id/submit` 폐기 (v1.6.4 결재 분리). `POST /settlements/empty` + `PATCH /settlements/transactions/:txId` + `PATCH /settlements/:id/title` 추가. ④ §5.5 — **`/link-approval`, `/unlink-approval` webhook 신규** (v1.6.4 approval → expense 연결 routing). 통신 방향 정리 갱신. ⑤ §6.3 — SourceOwnership 활용 분기 표 추가 (CORPORATE는 환급 대상 자동 제외). ⑥ §7 — `/expense/categories` 페이지 폐기, §7.0 매뉴얼 v1.4 (사업/구분/상세내역, 자동 제목 폐기, 탭 단순화, §11 지출결의서 가이드 인라인) 반영. §7.2 와이어프레임 컬럼 정정 (카테고리→사업/구분/상세). ⑦ §12.1bis v0.5 후속 작업 6건 추가. ⑧ §13 Risks — 카테고리·RabbitMQ 항목 폐기 처리 + cross-service contractId 무결성 위험 추가. ⑨ §14 O3 정산 묶음 분할 해결. ⑩ §5.3/§5.4 — G-4 `/admin/settlements`, G-6 `/internal/settlement-summary` 백로그 명시. | yunsim + Claude |
