...
1. `/repair` → **`+ AS 접수`** 버튼 클릭
2. 신규 접수 폼에서 다음 항목 입력:
   - 🔴 **고객** (필수) — 고객사 선택. 없으면 `/repair/customers`에서 먼저 등록
   - **장비/센서** — 고객 자산 목록에서 선택. 없으면 아래 제품 정보 직접 입력
   - **제품명 / 제조사 / 시리얼** — 자산 미등록 시 텍스트 입력
   - **증상** — 고장 현상 설명
   - **고객사 담당자명 · 연락처** — 필요 시
   - **접수자** — 본인 이름 기본값
   - **우선순위** — LOW / NORMAL / HIGH / URGENT
3. 저장 → **`AS-YYYY-NNNN` 접수번호** 자동 부여
4. 초기 상태: **`접수(RECEIVED)`**
...
삭제가 안 되는 상태에서 실수한 경우:
1. `→ 취소` 전이 후 삭제 (일반적)
2. DB 관리자에게 문의 (Admin 수동 상태 복원)
