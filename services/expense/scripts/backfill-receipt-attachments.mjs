// 일회성 백필 — expense container 내부에서 실행
// 매칭된 영수증 → approval-service 첨부
...
async function uploadOne(documentId, uploadedBy, receiptId) {
  const r = await prisma.expenseReceipt.findUnique({ where: { id: receiptId } });
  if (!r) { console.log(`  ⚠ receipt ${receiptId} not found`); return; }
  const diskPath = path.join(UPLOAD_ROOT, r.storageKey);
  const buf = await fs.readFile(diskPath);
  // 주의: fastify-multipart는 파일 파싱 시점에 그 전 필드만 fields에 포함 — 텍스트 먼저 추가
  ...
  if (!res.ok) {
    ...
    console.log(`  ✗ ${r.originalFileName} HTTP ${res.status}: ${txt.slice(0, 120)}`);
    return;
  }
...
console.log(`백필 대상: ${settlements.length}건`);
for (const s of settlements) {
  ...
  if (receiptIds.size === 0) { console.log(`\n[${s.title}] 매칭 영수증 없음 — skip`); continue; }
  console.log(`\n[${s.title}] doc=${s.approvalDocumentId} / 영수증 ${receiptIds.size}매`);
