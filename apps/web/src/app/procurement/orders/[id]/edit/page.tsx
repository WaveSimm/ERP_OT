...
import { procurementApi, supplierApi, userManagementApi } from "@/lib/api";
...
export default function EditOrderPage() {
  ...
  // 발주 데이터 로드 + prefill (v1.6 2026-05-14)
  useEffect(() => {
    (async () => {
      try {
        const order = await procurementApi.getOrder(orderId);
        if (order.status !== "DRAFT") {
          setStatusError(`현재 상태(${order.status})에서는 편집할 수 없습니다. 초안(DRAFT) 상태에서만 가능합니다.`);
          setLoading(false);
          return;
        }
        // 결재자 이름 lookup용 (users 로드 안 됐을 수 있으니 별도 fetch)
        const uList = users.length > 0 ? users : await userManagementApi.members(true).catch(() => [] as any[]);
        const findName = (uid?: string | null) => uid ? (uList.find((u: any) => u.id === uid)?.name || "") : "";
        setForm({
          contractId: order.contractId || "",
          manufacturer: order.manufacturer || "",
          currency: order.currency || "EUR",
          orderDate: order.orderDate ? order.orderDate.slice(0, 10) : "",
          estimatedProductionEnd: order.estimatedProductionEnd ? order.estimatedProductionEnd.slice(0, 10) : "",
          ...
          approverName: findName(order.approverId),
          secondApproverId: order.secondApproverId || "",
          secondApproverName: findName(order.secondApproverId),
          thirdApproverId: order.thirdApproverId || "",
          thirdApproverName: findName(order.thirdApproverId),
        });
        ...
        setStatusError(e.message || "발주 조회 실패");
      } finally {
        setLoading(false);
      }
    })();
  ...
  const onSelectMaster = (idx: number, master: any | null) => {
    ...
    setItems(prev => {
      ...
      // 우선순위: 첫 활성 variant skuCode > masterCode (v1.6.1: modelName 폐기)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // v1.6 (2026-05-14): 발주 기본 정보 PATCH
      const data: any = {
        contractId: form.contractId,
        manufacturer: form.manufacturer,
        currency: form.currency,
        totalAmount: form.totalAmount,
        totalAmountKRW: form.totalAmountKRW ? Number(form.totalAmountKRW) : null,
        orderDate: form.orderDate || null,
        estimatedProductionEnd: form.estimatedProductionEnd || null,
        ...
        // v1.6 (2026-05-14): 발주별 결재라인 — null 허용 (해제 가능)
        // approverName: 사용자 삭제 후에도 결재 히스토리 보존 (2026-05-15)
        approverId: form.approverId || null,
        approverName: form.approverId ? (form.approverName || null) : null,
        secondApproverId: form.secondApproverId || null,
        secondApproverName: form.secondApproverId ? (form.secondApproverName || null) : null,
        thirdApproverId: form.thirdApproverId || null,
        thirdApproverName: form.thirdApproverId ? (form.thirdApproverName || null) : null,
      };
      await procurementApi.updateOrder(orderId, data);

      // items 재정의 (DRAFT 상태만 가능): 기존 items 삭제 후 재추가
      const current = await procurementApi.getOrder(orderId);
      for (const it of (current.items ?? [])) {
        await procurementApi.deleteOrderItem(it.id);
      }
      for (const i of items) {
        await procurementApi.addOrderItem(orderId, {
  ...
  if (loading) {
    return <div className="max-w-4xl p-6 text-gray-400">로딩중...</div>;
  }
  if (statusError) {
    return (
      <div className="max-w-4xl p-6">
        ...
        <button onClick={() => router.push(`/procurement/orders/${orderId}`)}
          className="mt-3 px-4 py-2 text-sm border rounded-lg hover:bg-gray-50">상세로</button>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-xl font-bold">발주 수정 (DRAFT)</h1>
      </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <FilterableSelect
              ...
              onChange={(v) => {
                // v1.6 (2026-05-14): 계약 변경 시 고객사 자동 prefill (비어있을 때만)
      ...
      <div className="flex justify-end gap-3">
        <button onClick={() => router.push(`/procurement/orders/${orderId}`)} className="px-6 py-2 border rounded-lg hover:bg-gray-50">취소</button>
