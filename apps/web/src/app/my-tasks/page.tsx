...
// 모듈 레벨 컴포넌트 — 함수 안에 두면 매 렌더마다 unmount/remount되어 blur 즉시 발생
function EditCell({
  editKey, display, inputType = "text", width = "w-full", align = "center",
  ...
  return (
    <span
      onClick={(e) => { e.stopPropagation(); if (!isSaving) setEditCell({ key: editKey, value: display.replace(/%|h$/g, "") }); }}
...
export default function MyTasksPage() {
  ...
  const load = async () => {
    ...
    setError("");
    try {
      setGroups(await myTasksApi.list());
    } catch (e: any) {
  ...
  const handleStatusChange = async (task: any, newStatus: string) => {
    ...
    try {
      await taskApi.update(task.project.id, task.taskId, { status: newStatus });
      await load();
            ...
            {FILTER_TABS.map((tab) => (
              <button
                ...
                className={`px-2.5 py-1 rounded text-xs font-medium transition-colors ${
                  filterStatus === tab.status ? "bg-white shadow text-gray-900" : "text-gray-500 hover:text-gray-700"
                }`}
              >
            ...
            {filteredGroups.map((group) => {
              ...
              return (
                <div key={group.project.id} className="relative">
                  {/* 삽입 위치 라인 — before */}
                  {linePos === "before" && (
                    <div className="absolute -top-0.5 left-0 right-0 h-0.5 bg-blue-500 rounded-full z-10 pointer-events-none" />
                  )}
                <div
                  ...
                  onDragOver={(e) => {
                    ...
                    if (dragOver?.id !== group.project.id || dragOver?.pos !== pos) {
                      setDragOver({ id: group.project.id, pos });
                    }
                  }}
                  onDragLeave={(e) => {
                ...
                >
                  ...
                  <div className="flex items-center border-b border-gray-100 bg-gray-50 hover:bg-gray-100 transition-colors">
                    ...
                    <div
                      ...
                      className="pl-2 pr-1 cursor-grab active:cursor-grabbing text-gray-300 hover:text-gray-500 select-none"
                      title="드래그로 순서 변경"
                    >⠿</div>
                    <button
                      onClick={() => setCollapsedProjects((prev) => {
                        const n = new Set(prev);
                        n.has(group.project.id) ? n.delete(group.project.id) : n.add(group.project.id);
                        return n;
                      })}
                  ...
                  {!isCollapsed && (
                    <>
                      ...
                      <div className="grid gap-0 px-4 py-1.5 border-b border-gray-100 bg-gray-50/30 text-[10px] font-semibold text-gray-400 uppercase tracking-wide"
                        ...
                        <span className="text-center">시작일</span>
                        <span className="text-center">종료일</span>
                        <span className="text-center">진행률</span>
                        <span className="text-center">배당율</span>
                      </div>
                          ...
                          return (
                            <div key={task.taskId}>
                                  ...
                                  <div className="flex items-center justify-center">
                                    {statusEditId === task.taskId ? (
                                      <select
                                        ...
                                        onKeyDown={(e) => e.key === "Escape" && setStatusEditId(null)}
                                        onClick={(e) => e.stopPropagation()}
                              ...
                              {task.mySegments?.map((seg: any) => {
                                ...
                                const allocVal = seg.allocationMode === "PERCENT"
                                  ? (seg.allocationPercent ?? 100)
                                  : (seg.allocationHoursPerDay ?? 8);
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    {/* Status (single-seg only — multi-seg shows on task header) */}
                                    <div className="flex items-center justify-center">
                                      {!multiSeg && (
                                        statusEditId === task.taskId ? (
                                          <select
                                    ...
                                    <div className="flex items-center justify-center">
                                      <EditCell
                                        editKey={`${segKey}:startDate`}
                                        display={seg.startDate ?? ""}
                                        inputType="date"
                                        width="w-28"
                                        editCell={editCell}
                                        setEditCell={setEditCell}
                                        saving={saving}
                                        onCommit={makeCommitHandler(task, seg)}
                                      />
                                    </div>
                                    ...
                                    {/* Progress — slider + editable number */}
                                    <div className="flex items-center justify-center gap-1.5 pr-1" onClick={(e) => e.stopPropagation()}>
                                      <input
                                        ...
                                        max={100}
                                        step={5}
                                        value={seg.progressPercent ?? 0}
                                        onChange={(e) => {
                                          // 로컬 상태 즉시 반영을 위해 groups를 낙관적 업데이트
                                          const v = Number(e.target.value);
                                          setGroups((prev) => prev.map((g) => ({
                                            ...g,
                                            tasks: g.tasks.map((t: any) =>
                                              t.taskId !== task.taskId ? t : {
                                                ...t,
                                                mySegments: t.mySegments.map((s: any) =>
                                                  s.segmentId !== seg.segmentId ? s : { ...s, progressPercent: v }
                                                ),
                                              }
                                            ),
                                          })));
                                      ...
                                      />
                                      <EditCell
                                        editKey={`${segKey}:progress`}
                                        display={`${seg.progressPercent ?? 0}%`}
                                        inputType="number"
                                        width="w-10"
                                        align="right"
                                        editCell={editCell}
                                    ...
                                    <div className="flex items-center justify-center">
                                      <EditCell
                                        editKey={`${segKey}:allocation`}
                                        display={`${allocVal}${allocUnit}`}
                                        inputType="number"
                                        width="w-12"
                                        align="center"
                                        editCell={editCell}
