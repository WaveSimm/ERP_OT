...
def build_report(con) -> str:
    ...
    if q(con, "SELECT COUNT(*) FROM files WHERE hash_head1mb IS NOT NULL")[0][0] > 0:
        ...
        P("### 도메인 예시: 선박 특별 검사 신청 업무 ★")
        P("")
        P("> 오션테크는 해양 관측·조사 사업 시 자체 작업선 외에 **일반 어선을 임대해**")
        P("> **작업용 특수 선박으로 일시 용도 변경**합니다 (선박 특별 검사 신청).")
        P("> §7 Top 30의 선적증서·어선검사증서·선박서류 시리즈는 모두 이 업무의 흔적입니다.")
        P(">")
        P("> 주력 임대 작업선:")
        P("> - **만성호** 229회 / 13개 부서·기관 ★ 사실상 회사 차원의 주력선")
        P("> - **일진호** 178회 / 11개 부서·기관")
        P("> - **씨로드호** 85회 / 12개 부서·기관")
        P("> - 은하호·스피드호·럭키2호·동진호·파이넥스호·제2만성호 등")
        P(">")
        P("> 자동화 후보: ERP **선박 자산 마스터** + 선박별 표준 서류 묶음 →")
        P("> 프로젝트에서 \"작업선: 만성호\" 선택 시 최신 선적증서·어선검사증서·")
        P("> 서류 통합본 자동 첨부. 어선검사증서는 연차 갱신 자동 버전 통제.")
        P(">")
        P("> 도메인 메모리: `project_special_vessel_inspection`")
        P("")
        P("### 시사점")
        P("")
